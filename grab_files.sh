...
website="$1"
autograb_links="${2:-1}" # If arg 2 is missing, set it to default (to automatically crawl the webpage, create a list of downloadable files and then grab them)
...
# Fetch the page
curl -s -b cookies.txt -L -o page_to_grab.html "$website"


# Extract file links
if [ "${autograb_links:-0}" -eq 0 ]; then
    echo -e "\nDownloading files using the button for batch downloading the parent folder...\n"
    url=$(grep -oP "<a\s+href=['\"]\K[^'\"]+(?=['\"][^>]*>\s*<span[^>]*title=['\"]Λήψη όλου του καταλόγου)" page_to_grab.html | sed 's/&amp;/\&/g')
    ...
    my_unzipped_folder="catalog_$(date +%Y_%m_%d___%H_%M_%S)"
    mkdir -p $my_unzipped_folder && curl -i -L -b cookies.txt -o temp.zip "$full_url" && unzip -q temp.zip -d $my_unzipped_folder
    mapfile -t urls < <(unzip -Z1 temp.zip)
    rm temp.zip
    zipped_files_count=$(find "$my_unzipped_folder" -maxdepth 1 -type f \( -iname "*.zip" -o -iname "*.rar" \) | wc -l)
    # exit 0
else
    mapfile -t urls < <(grep -oP "href=['\"]\K[^'\"]+\.(pdf|zip|rar|tar|gz|docx?|xlsx?|pptx?|txt|csv|7z|tar\.gz|tgz|mp4|mp3)" page_to_grab.html)
fi
...
for url in "${urls[@]}"; do
    ...
    if [ "${autograb_links:-0}" -eq 0 ]; then
        ...
        # Download
        if curl -s -b cookies.txt -L -O "$full_url"; then
